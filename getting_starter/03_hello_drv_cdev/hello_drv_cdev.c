...
// 自动生成主设备号
static unsigned int hello_drv_cdev_major;
// 内核空间buf
 ...
 */
static ssize_t cdev_write(struct file *filp, const char __user *buf, size_t size,
                   loff_t *offset) {
...
static int hello_cdev_init(void) {
    ...
    hello_drv_cdev_major = MAJOR(dev);
    if (ret < 0) {
    ...
    // 创建device，如下操作会让系统自动生成一个 /dev/hello_drv_cdev_device
    // 的设备节点
    device_create(hello_drv_cdev_class, NULL, MKDEV(hello_drv_cdev_major, 0),
                NULL, "hello_drv_cdev_device");
...
static void hello_cdev_exit(void) {
    device_destroy(hello_drv_cdev_class, MKDEV(hello_drv_cdev_major, 0));
